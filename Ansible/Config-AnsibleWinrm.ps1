...
START CMD "winrm create winrm/config/Listener?Address=IP:*+Transport=HTTPS @{Hostname=`"$env:COMPUTERNAME`";CertificateThumbprint=`"$thumbprint`"}"
...
Setup-AnsibleWinRM -CertValidityDays 1095 -SkipNetworkProfileCheck -ForceNewSSLCert -GlobalHttpFirewallAccess -EnableCredSSP -Verbose -ErrorAction Continue